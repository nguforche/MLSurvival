...
#' @rdname MLSurvival
#' @export
MLSurvival <-  function(form, dat, method, predict.times, trControl, parallel = FALSE, dummy.vars = TRUE, mc.cores = 2, 
                        seed = 123, ...){				
  dummy.model <- NULL 
  if(dummy.vars){
  ...
  if(length(fac.vars) > 0){
    ...
    dummy.model <- dummyVars(formula = form, data = dat) ### model to convert factors to dummy variables 
...
Classifier = {model <- train_classifier(form=form, dat=dat, method = mod1, predict.times=predict.times, trControl=trControl, 
                    parallel = parallel, mc.cores = mc.cores, seed = seed, ...)	
#cat("done classifier \n")
}, 
...
perf <- Performance(object=model, method=mod)
rownames(perf) <- NULL 
perf.ave <- getResults.ci2(perf, alpha = 0.05, groups = c("method", "predict.times") )
rownames(perf.ave) <- NULL 

cat("done", mod1, "\n")

return(list(model = model, perf = perf, perf.ave = perf.ave))
})
names(res) <- method
res 
}  
